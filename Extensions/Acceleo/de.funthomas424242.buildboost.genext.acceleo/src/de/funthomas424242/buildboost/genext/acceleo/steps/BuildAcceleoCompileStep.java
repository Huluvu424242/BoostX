...
public class BuildAcceleoCompileStep extends AbstractAntTargetGenerator {
	...
	public BuildAcceleoCompileStep(
			EclipseUpdateSiteDeploymentSpec updateSiteSpec, File targetDir) {
		super();
	...
	public Collection<AntTarget> generateAntTargets() throws BuildException {
		if (usernameProperty == null) {
			usernameProperty = updateSiteSpec.getValue("site",
					"usernameProperty");
			System.out.println("Using user: " + usernameProperty);
		}
		if (passwordProperty == null) {
			passwordProperty = updateSiteSpec.getValue("site",
					"passwordProperty");
			System.out.println("Using password: " + passwordProperty);
		}

		AntTarget acceleoCompileTarget = generateAcceleoCompileAntTarget();
		return Collections.singletonList(acceleoCompileTarget);
	}

	protected AntTarget generateAcceleoCompileAntTarget() throws BuildException {
		EclipseUpdateSite updateSite = updateSiteSpec.getUpdateSite();
		if (updateSite == null) {
			throw new BuildException(
					"Can't find update site for update site specification.");
		}
		...
		String jarsDir = distDir + File.separator + "jars" + File.separator
				+ repositoryID;
		String mavenRespoitoryDir = distDir + File.separator
				+ "maven-repository" + File.separator + repositoryID;
		...
		for (EclipseFeature feature : updateSite.getFeatures()) {
			String featureVersion = updateSiteSpec.getFeatureVersion(feature
					.getIdentifier());
			for (Plugin plugin : feature.getPlugins()) {
				String pluginID = plugin.getIdentifier();
				String pluginVersion = updateSiteSpec.getValue("plugin",
						pluginID, "version");
				if (pluginVersion == null) {
					pluginVersion = featureVersion;
				}
				plugin2VersionMap.put(pluginID, pluginVersion);
				addDependenciesRecursively(plugin, pluginsToRepack,
						plugin2VersionMap);
			}
		}
		...
		for (EclipseFeature feature : updateSite.getFeatures()) {
			String featureVersion = updateSiteSpec.getFeatureVersion(feature
					.getIdentifier());
			String featureVendor = updateSiteSpec.getFeatureVendor(feature
					.getIdentifier());
			for (Plugin plugin : feature.getPlugins()) {
				String pluginID = plugin.getIdentifier();
				...
				String pluginVendor = updateSiteSpec.getValue("plugin",
						pluginID, "vendor");
				if (pluginVendor == null) {
					pluginVendor = featureVendor;
				}
				String pluginName = updateSiteSpec.getValue("plugin", pluginID,
						"name");
				if (pluginName == null) {
					pluginName = idToName(pluginID);
				}

				String snapshotValue = updateSiteSpec.getValue("site",
						"snapshot");
				...
				String pomXMLContent = generateEmtlContent(plugin,
						pluginVersion, pluginName, pluginVendor,
						plugin2VersionMap);
				if (pomXMLContent == null) {
					continue;
				}
				String pomPropertiesContent = generatePomProperties(plugin,
						pluginVersion);

				String pomFile = writeEmtlFile(pomXMLContent, pluginDirectory,
						pluginID, "xml").getAbsolutePath();
				writeEmtlFile(pomPropertiesContent, pluginDirectory, pluginID,
						"properties");

				// package plugin(s)
				content.append("<echo message=\"Compile acceleo templates '"
						+ pluginID + "'\"/>");
				String destBinJarFile = jarsDir + "/" + pluginID + "-"
						+ pluginVersion + ".jar";
				...
				content.append("</jar>");
				String destSrcJarFile = jarsDir + "/" + pluginID + "-"
						+ pluginVersion + "-sources.jar";
				...
				for (File childFolder : pluginDirectory.listFiles()) {
					...
					if (childFolder.isDirectory()
							&& childFolder.getName().startsWith("src")) {
						content.append("<fileset dir=\""
								+ childFolder.getAbsolutePath() + "\"/>");
					}
				}
				content.append("</jar>");

				deployBinInRepository(content, jarsDir, mavenRespoitoryDir,
						pomFile, destBinJarFile);
				deploySrcInRepository(content, jarsDir, mavenRespoitoryDir,
						plugin, pluginVersion, destSrcJarFile);
		...
		for (CompiledPlugin compiledPlugin : pluginsToRepack) {
			...
			String pomXMLContent = generateEmtlContent(compiledPlugin,
					pluginVersion, pluginName, pluginVendor, plugin2VersionMap);
			if (pomXMLContent == null) {
				continue;
			}
			String pomPropertiesContent = generatePomProperties(compiledPlugin,
					pluginVersion);

			String dirName = compiledPlugin.getFile().getName()
					.replace(".jar", "");
			File pluginDirectory = new File(compiledPlugin.getFile()
					.getParent(), dirName);
			String emtlFile = writeEmtlFile(pomXMLContent, pluginDirectory,
					pluginID, "xml").getAbsolutePath();
			writeEmtlFile(pomPropertiesContent, pluginDirectory, pluginID,
					"properties").getAbsolutePath();

			content.append("<echo message=\"Repacking for maven repository '"
					+ pluginID + "'\"/>");
			String destBinJarFile = jarsDir + "/" + pluginID + "-"
					+ pluginVersion + ".jar";
			content.append("<jar destfile=\"" + destBinJarFile + "\">");
			...
			// src version
			String srcPluginFile = compiledPlugin.getFile().getName()
					.replace(pluginID, pluginID + ".source");
			pluginPath = new File(compiledPlugin.getFile().getParent(),
					srcPluginFile).getAbsolutePath();
			String destSrcJarFile = jarsDir + "/" + pluginID + "-"
					+ pluginVersion + "-sources.jar";
			content.append("<jar destfile=\"" + destSrcJarFile + "\">");
			content.append("<zipfileset src=\"" + pluginPath + "\">");
			...
			// Ab hier compilieren
			deployBinInRepository(content, jarsDir, mavenRespoitoryDir,
					emtlFile, destBinJarFile);
			deploySrcInRepository(content, jarsDir, mavenRespoitoryDir,
					compiledPlugin, pluginVersion, destSrcJarFile);
		...
		if (deployedSomething) {
			...
			String repoPath = targetPath.substring(0,
					targetPath.lastIndexOf('/') + 1)
					+ "maven-repository";

			content.append("<scp todir=\"${env." + usernameProperty
					+ "}:${env." + passwordProperty + "}@" + repoPath
	...
	protected void deploySrcInRepository(XMLContent content, String jarsDir,
			String mavenRespoitoryDir, Plugin plugin, String pluginVersion,
			String destSrcJarFile) {

		content.append("<acceleo blahexec executable=\"mvn\" dir=\"" + jarsDir
				+ "\">");
		content.append("<arg value=\"deploy:deploy-file\"/>");
		...
		for (IDependable dependency : plugin.getDependencies()) {
			if (dependency instanceof CompiledPlugin) {
				...
				if (includeInMavenRepository(compiledPlugin)) {
					plugin2VersionMap.put(compiledPlugin.getIdentifier(),
							compiledPlugin.getVersion());
					pluginsToRepack.add(compiledPlugin);
					addDependenciesRecursively(compiledPlugin, pluginsToRepack,
							plugin2VersionMap);
				}
			}
		}
	...
	protected String generateEmtlContent(Plugin plugin, String pluginVersion,
		...
		content.append("<groupId>" + getMavenGroup(plugin.getIdentifier())
				+ "</groupId>");
		content.append("<artifactId>" + plugin.getIdentifier()
				+ "</artifactId>");
		...
		for (UnresolvedDependency dependency : plugin.getResolvedDependencies()) {
			...
			String dependencyVersion = plugin2VersionMap.get(dependency
					.getIdentifier());
			if (dependencyVersion == null) {
				System.out.println("Can not create maven artifact for "
						+ plugin.getIdentifier() + " since "
						+ dependency.getIdentifier() + " is not versioned");
				return null;
			}
			content.append("<dependency>");
			content.append("<groupId>"
					+ getMavenGroup(dependency.getIdentifier()) + "</groupId>");
			content.append("<artifactId>" + dependency.getIdentifier()
					+ "</artifactId>");
	...
	protected File writeEmtlFile(String pomXMLContent, File pluginDirectory,
			String pluginID, String extension) {
		File pomFolder = new File(pluginDirectory.getAbsolutePath()
				+ File.separator + "META-INF" + File.separator + "maven"
				+ File.separator + getMavenGroup(pluginID) + File.separator
				+ pluginID);
		pomFolder.mkdirs();
	...
	protected boolean isOptional(UnresolvedDependency dependency) {
		...
		// This dependency is not marked optional in EMF plugins although it is
		// optional.
	...
	protected boolean includeInMavenRepository(CompiledPlugin compiledPlugin) {
		// TODO This selects the EMF core plugins, could be moved to an external
		// spec
